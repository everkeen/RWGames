export class Particle {
    ...
    any1 = null; // Shared Any Value 1, can hold any type of data, not affected by game, only behaviors
    any2 = null; // Shared Any Value 2, can hold any type of data, not affected by game, only behaviors
    any3 = null; // Shared Any Value 3, can hold any type of data, not affected by game, only behaviors
    any4 = null; // Shared Any Value 4, can hold any type of data, not affected by game, only behaviors
    any5 = null; // Shared Any Value 5, can hold any type of data, not affected by game, only behaviors
...
export function energyBehavior(game, particle, onBounce) {
...
export class CPURenderer extends Renderer {
    ...
    get supportsSpecialGraphics() {
        return true;
    }
    get supportsParticles() {
        return true;
    }
}
export class GPURenderer extends Renderer {
    ...
    gridShaderProgram = null;
    brushPreviewProgram = null;
    framebuffer = null;
    cellWidth = 0;
    cellHeight = 0;
    init(game) {
    ...
    renderGrid(game) {
        if (!this.ctx || !this.gridShaderProgram)
            return;
        const gl = this.ctx;
    ...
    renderParticles(game) {
        if (!this.ctx || !this.particleShaderProgram)
            return;
        const gl = this.ctx;
        const canvasW = game.canvas.width;
        const canvasH = game.canvas.height;
...
export class Powders {
    canvas;
    renderer;
    rendererOption = "auto"; // "auto", "gpu", or "cpu"
    grid;
    mouseX = 0;
    mouseY = 0;
    lastMouseX = 0;
    ...
    doDebugRender = false;
    isMobile;
    particleLimit = 1000; // Null for unlimited
    particlesEnabled = true; // Particles enabled?
    particles = [];
    ...
    highQualityExplode = false; // Makes explosions higher quality (64 rays instead of 32)
    debugRenderShapes = [];
    set noInput(value) {
        this._noInput = value;
        if (!value) {
    ...
    selectorMatch(selector, particle) {
    ...
    parseSelector(selector) {
        ...
        for (const typeId of typeIds) {
            const processed = this.processTypeId(typeId.trim(), null);
    ...
    initSettings() {
        ...
        const highQualityExplodeCheckbox = document.getElementById("settings-high-quality-explode");
        ...
        const rendererSelect = document.getElementById("settings-renderer");
        rendererSelect.value = this.rendererOption;
        rendererSelect.onchange = () => {
            this.rendererOption = rendererSelect.value;
            this.saveSettings();
            alert("Renderer option changed. Please refresh the page for the change to take effect.");
        };
        const particleLimitSelect = document.getElementById("settings-particle-limit");
    ...
    getSetting(key, fromLocal = false) {
        if (fromLocal) {
            const settings = this.getLocalStorageSettings();
    ...
    canSwap(type, x, y, sameTypeSwap = false) {
        if (this.isFree(x, y)) {
            return true;
        }
    ...
    spawnParticle(x, y, type, replace = false, forceMainGrid = false) {
    ...
    removeParticle(x, y, forceMainGrid = false) {
...
function wavelengthEnergyToColor(wavelength, energy, useOutOfRange = false) {
...
function colorToWavelengthEnergy(color) {
...
powderTypes.register("supernova", {
    ...
    behavior: (game, particle) => {
        ...
        for (const affectedParticle of affected) {
            if (affectedParticle === null)
                continue;
            if (affectedParticle.particle === null)
                continue;
...
function isValidPhotonColorProfile(data) {
    if (!Array.isArray(data))
        return false;
    return data.every((item) => item !== null &&
        typeof item === "object" &&
        typeof item.wavelength === "number" &&
        typeof item.energy === "number");
}
function interpolatePhotonProfile(profile, targetWavelength) {
    if (!profile || profile.length === 0)
        return 1;
    if (profile.length === 1)
        return profile[0].energy;
...
powderTypes.register("photon", {
    ...
    colorVariation: 0.1,
    behavior: (game, particle) => {
        const onBounce = (game, particle, bouncedOn) => {
            if (!bouncedOn)
                return;
            ...
            if (bouncedOn.type === "photon")
                return; // Photons dont interact with each other, they just pass through each other
            const bouncedOnType = bouncedOn.type ? powderTypes.require(bouncedOn.type) : null;
            if (!bouncedOnType)
                return;
            if (bouncedOnType.noTransfer !== undefined && bouncedOnType.noTransfer)
                return;
